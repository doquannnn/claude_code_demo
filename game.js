...
for (let r = 0; r <= N - K; r++)
  for (let c = 0; c <= N - K; c++)
    WINNING_LINES.push(
      Array.from({ length: K }, (_, k) => (r + k) * N + (c + k)),
    );
for (let r = 0; r <= N - K; r++)
  for (let c = K - 1; c < N; c++)
    WINNING_LINES.push(
...
function scoreFor(board, idx, player) {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    const fwdOpen =
      r >= 0 && r < N && c >= 0 && c < N && board[r * N + c] === null;
    if (fwdOpen) openEnds++;
    // extend backward
    r = r0 - dr;
    c = c0 - dc;
...
function aiMove(board) {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    const distToCenter =
      Math.abs(((i / N) | 0) - Math.floor(N / 2)) +
      Math.abs((i % N) - Math.floor(N / 2));
    if (
      score > bestScore ||
      (score === bestScore &&
        distToCenter <
          Math.abs(((bestIdx / N) | 0) - Math.floor(N / 2)) +
            Math.abs((bestIdx % N) - Math.floor(N / 2)))
    ) {
      bestScore = score;
      bestIdx = i;
    }
